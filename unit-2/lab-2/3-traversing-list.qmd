...
::: learn
<p><strong>On this page,</strong> you will complete your quiz app by using a new block that loops through a list:<br /> <img src="/img/2-complexity/for-each-(item)-of().png" title="for-each-(item)-of(){}" alt="for-each-(item)-of(){}" class="ms-4" height="68"/></p>
:::
...
::: vocabFullWidth
<p>Using the result from `item` as the input to `address from contact` is called <strong>composition</strong> of functions.</p>
:::
...
<p><strong>Traversing</strong> a list means looking at each item of the list. `For each` is iterative. That is, it's repetitive, like `for`, which can also traverse a list. But unlike `for`, `for each` traverses the list <em>without using index numbers</em>.</p>
:::
...
<li>Use `for each` to <em>traverse</em> your list, asking each quiz question, checking each answer, and letting the user know if they are right or wrong on each item.</li>
...
::: prompt
<p>Which of the scripts below will produce the following sprite behavior?<br /> <img src="/img/3-lists/abelincoln.gif" title="sprite saying Abraham then Lincoln" alt="sprite saying Abraham then Lincoln" class="ms-4 noshadow"/><br /> (When you run the script, though, it actually says Abraham Lincoln <em>only once</em>.)</p>
