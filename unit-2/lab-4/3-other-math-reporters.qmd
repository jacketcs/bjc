...
::: takeNote
<p><strong>You choose the operation, and `combine` performs
...
::: endnote
<p>`Combine` is a <em>higher-order function</em>; it is a
...
<p>`Combine` is mostly used with <em>only these
functions</em>:<br />
...
<p>`Average of list` and `sum of list` are
...
```
(like `report`). For example, this definition<br />
...
::: pseudop
As with `report` in Snap<em>!</em>, when a
