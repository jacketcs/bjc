...
<li>Experiment with the <img src="/img/blocks/switch-to-costume.png" title="switch to costume []" alt="switch to costume []" class="inline py-0"/> block using different inputs to switch the sprite's costume manually a few times.</li>
...
<p>When you used `item of` before (for example, inside `who` and `does what`) , you were accessing items of the list <em>randomly</em>. But you can also select a specific item by specifying its <em>position</em>.</p>
...
<p>And you can use `item of` together with `my` to select a costume in a specific position in the list.</p>
...
<li>Use <img src="/img/blocks/show-variable-full-size.png" title="show variable []" alt="show variable []" class="inline py-0"/> to show the contents of that variable.</li>

<li>`Ask` the player "Which costume number do you want?"</li>

<li>Change to the costume with the index number in the player's `answer`.</li>
