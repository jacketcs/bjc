...
<p>You can improve `plural` to work correctly with more
words. If the project is not already open, please reload it. (The
suggested file name was `U2-Plural`.)</p>
...
::: endnote
<p>Right now, if `plural` is given a word with a space at the
...
<p>Surprise! Once `plural` works for a single space at the
...
<p>`Find first` is a higher-order function works similarly to
`keep`, but it reports only the first item that's found. It
is equivalent to `item (1) of (keep)`.</p>



<p>The `call` block in the palette doesn't include the
