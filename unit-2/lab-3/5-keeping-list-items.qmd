...
::: learn
<p><strong>On this page</strong>, you will use predicates with the `keep` block to keep items in a list that have specific characteristics.</p>
:::
...
::: takeNote
<p><strong>You write the predicate that does the checking, and `keep` applies the predicate to each item in the input list and then reports the list of items that make your predicate `true`</strong>.</p>
...
<p>There are two `length` blocks in Snap<em>!</em>. One (<img src="/img/blocks/length-of-blank-operator.png" title="length ()" alt="length ()" class="inline"/>) is designed for strings, and the other (<img src="/img/blocks/length-list.png" title="length ()" alt="length ()" class="inline"/>) is designed for lists.</p>
