...
<li>Give your user some coloring options. For example, when the "1" key is pressed, the pen color is reset to black; when the "2" key is pressed, the pen color is set to red; etc. (Suggestion: Load the crayon library ("Provide 100 selected colors.") Also consider using some keys to put the `pen up` or `pen down`.</li>
