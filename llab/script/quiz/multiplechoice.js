...
function MC(data, location, questionNumber) {
    ...
    var rii = this.interaction.attr("data-responseIdentifier");
    this.responseDec = $('.responseDeclaration[data-identifier="' + rii + '"]');
...
MC.prototype.loadContent = function() {
    ...
    this.properties.shuffle = this.interaction.attr('data-shuffle') == "true";
    this.properties.maxChoices = this.interaction.attr('data-maxchoices');

    // get the list of correct responses
    var corrResponses = this.responseDec.find('.correctResponse');

    for (i = 0; i !== corrResponses.length; i++) {
        this.correctResponse.push($(corrResponses[i]).attr('data-identifier'));
    }
...
MC.prototype.checkAnswer = function() {
    ...
    for (i = 0; i < inputbuttons.length; i++) {
        ...
        if (checked) {
            if (choice) {
                ...
                this.multipleChoice.find('#feedback_' + choiceIdentifier).addClass("question-feedback");
    ...
    var headerdiv = this.multipleChoice.find('.card-header');
    var outerdiv = headerdiv.parent();
        // Remove the confirmation classes if previously added.
    outerdiv.removeClass('border-primary');
    outerdiv.removeClass('border-success');
    outerdiv.removeClass('border-danger');

    headerdiv.removeClass('bg-primary');
    headerdiv.removeClass('bg-success');
    headerdiv.removeClass('bg-danger');
    if (isCorrect) { //the student answered correctly
        outerdiv.addClass('border-success');
        headerdiv.addClass('bg-success');
...
MC.prototype.clearFeedbackDiv = function() {
    ...
    this.multipleChoice.find('.feedbackdiv').removeClass("question-feedback");
...
MC.prototype.getTemplate = function() {
    return "<div class='card MultipleChoice Question'>" +
        ...
        "        <div class='list-group list-group-flush currentQuestionBox'>" +
