...
list (and says `false`). It is still a linear search.</p>
...
Compare this `binary search` block with your code from
<a href="/unit-5/lab-1/1-sorted-lists.html" title="Page 1: Guess My Number">Page
...
<li>Try `binary search` with some words that you know are
...
<li>Now use `binary search` to search for the same words in
...
<p>The `contains` block can't make any assumptions about the
