...
::: learn
<p><strong>On this page</strong>, you'll implement the Tic-Tac-Toe
strategy rules to make the computer play strategically.</p>
:::
...
<var>position number</var> will make the move.</p>

<p>

The code for <code>next move for computer</code> will implement the
...
that into code using the <code>if</code> block, you'll need a predicate
to put in the hexagonal input slot of <code>if</code>. Write
...
use it as the input to <code>broadcast</code> instead of <code>best
empty square</code>.</li>

::: endnote
<a href="#hint-rules" data-toggle="collapse" title="Click for a hint about implementing the strategy rules.">Click
...
picture:<br />
<img src="/img/hof/fork.png" title="board with fork" alt="board with fork" class="indent"/><br />
...
triples:<br />
<img src="/img/hof/ttt-flatten.png" title="script variable(atoms); set (atoms) to (combine with(append()()) items of (singles))" alt="script variable(atoms); set (atoms) to (combine with(append()()) items of (singles))" class="indent"/><br /></li>
