...
<p>Computers can be useful in processing data to gain information, but
your ability process data depends both on your capabilities and the
tools you have available.</p>

<p><strong>On this page</strong>, you will build various tools
...
<p><img src="/img/5-algorithms/correlation-positive-trend.png" title="a graph with a bunch of data points, in which they generally follow a straight line from top left to bottom right" alt="a graph with a bunch of data points, in which they generally follow a straight line from top left to bottom right" class="imageRight"/>
...
that works in any of these applications and in Snap<em>!</em>.</p>

::: endnote
<a href="#hint-CSV" data-toggle="collapse" title="What does CSV stand for">What
...
<p>Although spreadsheets are a common kind of data, they aren't the only
...
spreadsheets with multiple tabs.</p>

::: forYouToDo
<ol start="4">

<li>Experiment with different inputs to the
<img src="/img/blocks/item.png" title="item () of &apos;list input slot&apos;" alt="item () of &apos;list input slot&apos;" class="inline nopadtb"/>
...
How does the first item of your data set (<code>item (1) of</code>)
...
::: endnote
<p>A table is represented in Snap<em>!</em> as a list of lists. If you
...
<a href="#hint-views" data-toggle="collapse" title="See examples of table view and list view.">See
examples of table view and list view.</a></p>
...
::: vocabFullWidth
: <strong>Records</strong>, <strong>Fields</strong>, and
<strong>Columns</strong>

<ul>

<li>A <strong>record</strong> is one row in a dataset (other than the
...
<li>A <strong>field</strong> is one item of a record in a dataset. It
...
<li>A <strong>column</strong> is a list containing the data from one
...
<li>Build
<img src="/img/5-algorithms/record-of-table.png" title="record () of table ()" alt="record () of table ()" class="inline nopadtb"/>
and
<img src="/img/5-algorithms/field-of-record.png" title="field () of record ()" alt="field () of record ()" class="inline nopadtb"/>
...
Build
<img src="/img/5-algorithms/column-of-table.png" title="column () of table ()" alt="column () of table ()" class="inline nopadtb"/>
to report just one column of your dataset.

::: endnote
<p><code>Record</code> and <code>column</code> are <em>selectors</em>
for a <code>table</code> abstract data type. We don't need a
...
(<code>Field</code> is a selector for <code>record</code>, which is,
itself, an abstract data type.)</p>

<p>Notice that these suggested block names include the word "table" or
...
that doesn't match the input you want to use.</p>

<p><a href="#hint-map" data-toggle="collapse" title="Try to figure out how to report just one column, but click if you really need a hint.">Try
...
<a href="#hint-more-map" data-toggle="collapse" title="Have I seen map before?">Have
I seen <code>map</code> before?</a>

::: {#hint-more-map .collapse}
Since a table is a list of lists (where each inner list is one record),
<code>map</code> will perform the same function on every record in your
dataset. You will need to determine what function to map over the
dataset. You learned about <code>map</code> on
...
column</strong> of your data set, and then build code to answer your
question. You may need to build additional blocks or import them from
other projects.

::: endnote
<p>You can see the column number by holding your mouse pointer over the
...
<p>You may need to use <code>map</code>, <code>keep</code>, or
<code>combine</code> to answer your question.
<a href="#hint-HOFs" data-toggle="collapse" title="Click to see where you learned about these higher order functions.">Click
to see where you learned about these higher order functions.</a></p>

::: {#hint-HOFs .collapse}
<ul>

<li>You learned about <code>map</code> on
<a href="/unit-3/lab-2/5-mapping-over-list.html" title="Unit 3 Lab 2 Page 5: Transforming Every List Item">Unit
3 Lab 2 Page 5: Transforming Every List Item</a>.</li>

<li>You learned about <code>keep</code> on
<a href="/unit-2/lab-3/5-keeping-list-items.html" title="Unit 2 Lab 3 Page 5: Keeping Items from a List">Unit
2 Lab 3 Page 5: <code>Keep</code>ing Items from a List</a> .</li>

<li>You learned about <code>combine</code> on
...
<p><a href="#hint-questions" data-toggle="collapse" title="Click for example questions to ask about a single column.">Click
for example questions to ask about a single column.</a></p>

::: {#hint-questions .collapse}
<ul>

<li>What's the average MPG that cars in this database get in the city?
(You'd need an <code>average</code> block.)</li>

<li>What's the year of the oldest car in this dataset? (You'd need a
<code>minimum</code> block.)</li>

<li>How many cars in this dataset have manual transmission?</li>

</ul>

<p>Notice that all of these examples only require data from one column.
...
can do the Take It Further Activity below.</p>
:::
:::
...
<p>Researchers often face challenges with data before they even begin
...
or invalid (if some people made mistakes).</p>
...
and clean digital data, thereby gaining insight and knowledge.</p>
:::
...
<p>Imagine you read in the news that people who eat a lot of broccoli
...
reasons the data might not mean what they seem to mean.</p>

<p>People sometimes think that the way to overcome bias is to use a
...
::: todo
<p>I need to redo these images and <code>code</code> statements with
"record" instead of "row." --MF, 6/27/19</p>
:::
...
MPG?" by building a
<img src="/img/5-algorithms/row-with-max-of-table.png" title="row with maximum in column () of table ()" alt="row with maximum in column () of table ()" class="inline nopadtb"/>
block.
...
```
<p><a href="#hint-build" data-toggle="collapse" title="Click for a hint about how to build row with maximum in column () of table ().">Click
for a hint about how to build <code>row with maximum in column () of
table ()</code>.</a></p>
...
<img src="/img/5-algorithms/maximum-of-list.png" title="maximum of &apos;list input slot&apos;" alt="maximum of &apos;list input slot&apos;" class="inline nopadtb"/>
function for a simple list (that isn't a table) is shown below. You can
build a
<img src="/img/5-algorithms/maximum.png" title="maximum of () and ()" alt="maximum of () and ()" class="inline nopadtb"/>
block that compares two inputs and use it with <code>combine</code> to
...
report (combine (data) using (maximum of ( ) and ( )))
}" class="indent"/><br /> You can use a similar approach here by first
building a
...
with the higher value in the specified column.</p>
:::
