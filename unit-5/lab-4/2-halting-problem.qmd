...
::: todo
<p>PG: This is a lovely piece of logic, well written. I'd love to keep
...
approach. Does it work?</p>

<p>BH: But FYTD 1c is kinda TIFfy. And, I think Church actually proved
it before Turing, but we should check. And, bring back his picture!</p>

<p>MF: I want to review this page just because the idea deserves it</p>
:::

::: learn
<p><strong>On this page,</strong> you will consider a problem that
<em>can't</em> have an answer.</p>
:::

<p>By this point in the course, you've experienced the frustration of
...
<p>First, we'll assume that it's possible to write a function to
...
is wrong---<strong>no general-purpose debugging program can
exist</strong>. "Does this program have bugs" is an <em>undecidable
problem</em>; no algorithm can ever be constructed that always solves it
correctly for every program.</p>

::: vocabFullWidth
<p>An <strong>infinite loop</strong> is a sequence of computer
instructions that repeats forever.</p>

<p>An <strong>unsolvable problem</strong> is one for which no algorithm
can ever be written to find the solution.</p>

<p>An <strong>undecidable problem</strong> is one for which no algorithm
...
should have a yes/no answer (such as: does my code have a bug?).</p>
:::

<h3>Assuming That a <code>Halts?</code> Function Exists</h3>

<h4><strong>Making the Assumption</strong></h4>

<p>

::: sidenote
We can't show the code inside <code>halts?</code>; we are proving that
it can't be written at all, no matter what code implements it.
:::

First, we <em>assume</em> there exists a function called
<code>halts?</code> that determines whether a given reporter will report
a value in a finite time. (If so, we say that the reporter
<em>halts.</em>) It takes two inputs: a function and an input value for
that function. It reports <code>true</code> if the function would report
a value when given that input; it reports <code>false</code> if the
function would run forever (in an infinite loop).<br />
<img src="/img/5-algorithms/halts.png" title="halts? (function) (input)" alt="halts? (function) (input)" class="indent"/>

</p>

<p>For example, the function <code>round</code> will not run forever
when given the input <var>7.5</var>; it will report 8.</p>

::: sidenote
The gray ring around the input function means that the
<code>halts?</code> code will treat the input function as data instead
of running it.
:::
...
<p>On the other hand, imagine we had a function that will get stuck in
...
<h4><strong>Reviewing the Plan for the Proof</strong></h4>

<p>We'll use proof by contradiction to show that the following question
<em>can't</em> be answered <em>in general</em>:</p>

<p class="center">

<strong>Will a certain computer program, given a certain input, report a
result in a finite amount of time?</strong>

</p>

::: endnote
<p>The "computer program" is the first input to <code>halts?</code>. The
"given input" is the second input to <code>halts?</code>. And what
<code>halts?</code> reports is either <code>true</code> or
<code>false</code>: yes the "computer program" will halt or no it
won't.</p>

<p>Note that <code>halts?</code> itself must always report a result,
even if the program it's testing doesn't. This is because we are
<em>assuming</em> (an assumption we'll show is false) that
<code>halts?</code> always works---that it always reports an answer.</p>
:::

<p>We <em>can</em> answer this question for some <em>specific</em>
...
function will report a value for <em>any</em> specific input.</p>

::: endnote
<a href="#hint-general" data-toggle="collapse" title="What does it mean to answer the question for specific functions?">What
does it mean to answer the question for <em>specific</em> functions?</a>

::: {#hint-general .collapse}
<p>The "in general" part is important. Of course we can answer the
question for certain particular programs. This one halts (reports a
value) for <em>any</em> input:<br />
<img src="/img/5-algorithms/three-fn.png" title="three(x): report (3)" alt="three(x): report (3)" class="indent"/></p>

<p>This one <em>never</em> halts for any input:<br />
<img src="/img/5-algorithms/loop-fn.png" title="loop(x): forever (wait 1 secs)" alt="loop(x): forever (wait 1 secs)" class="indent" width="158"/></p>

<p>And this one halts <em>sometimes</em>---for any input except
87:<br />
<img src="/img/5-algorithms/weird-fn.png" title="weird(x): if (x = 87) (forever (wait 1 secs)) else (report (3))" alt="weird(x): if (x = 87) (forever (wait 1 secs)) else (report (3))" class="indent" width="158"/></p>

<p>Recall: the question is whether we can devise a test that will tell
us whether <em>any</em> particular function will report a value for
<em>any</em> specific input.</p>
:::
:::
...
<li>Describe how you are going use the <code>halts?</code> function in
...
<h4><strong>Constructing a Program For Which <code>Halts?</code> Won't
Work</strong></h4>
...
function and one input to that function for which <code>halts?</code>
fails to work the way we assumed it does. So, we can make up a block,
<code>tester</code>, specifically for the purpose of breaking
<code>halts?</code>:<br />

::: sidenoteBig
Just like the <code>\#</code> and <code>⋮</code> symbols for inputs
declared to be <em>numbers</em> or <em>lists</em>, the <code>λ</code> is
not part of the input's name but is a <strong>type hint</strong> that
was created when selecting the <em>reporter</em> input type:<br />\
<img src="/img/5-algorithms/input-type-reporter.png" title="selecting the Reporter input type" alt="selecting the Reporter input type" class="indent"/><br />
...
for <code>tester</code> will have a rounded gray ring to indicate that
the input should be a reporter:
<img src="/img/5-algorithms/tester().png" title="tester ()" alt="tester ()" class="inline nopadtb"/>
:::

<img src="/img/5-algorithms/tester-halting.png" title="tester(reporter){if(halts?(reporter)(reporter){forever{}}else{report(It doesn&apos;t matter what value is reported.)}}" alt="tester(reporter){if(halts?(reporter)(reporter){forever{}}else{report(It doesn&apos;t matter what value is reported.)}}" class="indent"/>

</p>

<p>The <code>forever</code> block in this script creates an infinite
loop. If the <code>tester</code> code ends up in this part of the
<code>if</code> statement, it will never report anything. So whether
<code>tester</code> <em>itself</em> will halt depends on the output of
the <code>halts?</code> predicate in the <code>if</code> statement
inside it.</p>
...
contradiction</em> to prove that the <code>halts?</code> function called
inside <code>tester</code> can't exist.
:::
...
<code>tester</code> on <em>itself</em>:<br />
<img src="/img/5-algorithms/testertester.png" title="tester(tester())" alt="tester(tester())" class="indent"/><br />
Now, the <code>if</code> statement inside the <code>tester</code> block
will ask if <code>tester</code> will halt (not run forever) if it's
called with <code>tester</code> as its input. The predicate in the
<code>if</code> statement will become <code>halts? (tester)
(tester)</code>.<br />

::: todo
I took out class="indent" on this picture so that it'd fit in the
...
<code>tester</code> definition:<br />
<img src="/img/5-algorithms/tester-halting.png" title="tester(reporter){if(halts?(reporter)(reporter){forever{}}else{report(true)}}" alt="tester(reporter){if(halts?(reporter)(reporter){forever{}}else{report(true)}}" class="indent"/></p>

<p>

Showing that <strong>the result has to be wrong</strong> involves a
...
then <code>tester</code> will take the first branch of the
<code>if</code>, and so it will loop forever. That means
<img src="/img/5-algorithms/testertester.png" title="tester(tester())" alt="tester(tester())" class="inline"/>
<em>won't</em> halt, and so <code>halts?</code> gave the wrong
answer.</li>
...
then <code>tester</code> will take the <code>else</code> branch and
...
::: endnote
It doesn't matter <em>what</em> value <code>tester</code> reports, just
that it reports <em>some</em> value, but it <em>does</em> matter what
value <code>halts?</code> reports.
:::

::: takeNote
<p>No matter what <code>halts?</code> reports, it will always disagree
with itself in a program like this. <strong>This contradiction (this
logical impossibility) means that the assumption that it's possible
write <code>halts?</code> has to be wrong</strong>. This isn't just a
...
decidable is the <em>Halting Theorem.</em></p>
:::
...
<code>halts?</code> does, and how <code>halts?</code> is going to be
used in the proof</li>

<li>The basics of how the <code>tester</code> function behaves (for
...
<p>Alan Turing (1912--1954) was a British computer scientist and
...
work regardless of future technology.</p>

<p>He proved that there are well-defined mathematical problems that
...
be the "father of modern computing."</p>

<p><a href="https://www.nytimes.com/2019/06/05/obituaries/alan-turing-overlooked.html" title="Article: Overlooked No More" target="_blank">Article:
Overlooked No More</a></p>
:::
