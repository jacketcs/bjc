...
<p>For example, the function `round` will not run forever
...
::: endnote
<p>The "computer program" is the first input to `halts?`. The
...
<h4><strong>Constructing a Program For Which `Halts?` Won't
Work</strong></h4>
...
<p>The `forever` block in this script creates an infinite
...
::: takeNote
<p>No matter what `halts?` reports, it will always disagree
...
write `halts?` has to be wrong</strong>. This isn't just a
