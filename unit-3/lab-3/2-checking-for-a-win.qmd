...
<p>`Map` applies the function in the gray ring to each item
...
<p>This `map` expression works the same way as the
...
<p>But here, the function in the gray ring is `item ( ) of
...
<p>Now you're going to use `status of triple` systematically
to check <em>all</em> the triples.</p>

<p>For example, Player O has won the game below, and the `status of
