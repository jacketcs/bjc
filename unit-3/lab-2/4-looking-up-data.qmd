...
Contact" button that uses a <code>find contacts</code> block to
...
<img src="/img/3-lists/find-contacts-with-name-Alphie-reporting.png" title="find contacts with name: (Alphie) reporting" alt="find contacts with name: (Alphie) reporting" class="indent"/></p>
:::

::: sidenote
You learned about
<a href="/unit-2/lab-3/5-keeping-list-items.html" title="Keeping Items from a List"><code>keep</code>
in Unit 2</a>.
:::
...
which reports <code>true</code> if the first string (for example, a
...
which returns <code>true</code> if the list is empty and
...
the sprite <code>say</code> each of the matching contacts.</li>
...
<li>Then, use your <code>find contacts</code> block to find all the
...
which contact they want to delete when <code>find</code> reports more
than one.

<ol>

<li>Write a helper block <code>delete contact</code> that takes a
...
sorted form) from the two contacts, and then use <code>\<</code> to
...
The <code>sort</code> block will report a new, sorted contact list.</li>
...
defined by the <code>name</code> ADT) has to know whether it's a
...
<li><img src="/img/icons/tough-stuff-mini.png" title="Tough Stuff" alt="Tough Stuff" class="inline"/>
Make sure your <code>find contacts</code> block and your sorting of the
